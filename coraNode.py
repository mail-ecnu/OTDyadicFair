...
def train_link_prediction_model(
    link_examples, link_labels, get_embedding, binary_operator, num_iter
):
    clf = link_prediction_classifier(max_iter=num_iter)
    link_features = link_examples_to_features(
...
def run_link_prediction(embedding_train, binary_operator, examples_train, labels_train, examples_model_selection, labels_model_selection, num_iter):
...
def edge_pred(examples_train, examples_model_selection, labels_train, labels_model_selection, embedding_train, num_iter=2000):
    results = [run_link_prediction(embedding_train, op, examples_train, labels_train, examples_model_selection,
            labels_model_selection, num_iter = num_iter) for op in binary_operators]
...
edge_weight = 0.15872583472031107
AUCs = []
ACCs = []
RBs = []
RB_ACCs = []
...
for i in range(5):
    ...
    sym_best_result = edge_pred(examples_train, examples_model_selection, labels_train, labels_model_selection, sym_embedding_train, num_iter=5000)
    ...
    xor_train_label, xor_val_label = get_xor_label(examples_train, graph_train), get_xor_label(examples_model_selection, graph_train)
    print('sym')
    best_ERB_result = edge_pred(examples_train, examples_model_selection, xor_train_label, xor_val_label, sym_embedding_train, num_iter=3000)
...
#     ERBs.append(erb_auc)
#     ERB_ACCs.append(erb_acc)
#     sym_di = DI(sym_best_result, examples_test, graph_test, sym_embedding_test)
#     print(f'sym_DI: {sym_di}')
#     print('Edge RB')
#     xor_train_label, xor_val_label = get_xor_label(examples_train, graph_train), get_xor_label(examples_model_selection, graph_train)
#     print('sym')
#     edge_pred(examples_train, examples_model_selection, xor_train_label, xor_val_label, sym_embedding_train)

# # dill.dump_session(f'./cora/new_{reg}.pkl')
