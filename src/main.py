...
from src.ot_laplace_clean import *
import os
import numpy as np
import importlib
import time
...
def total_repair_gwemd(g, metric='euclidean', log=False, name='plot_cost_gamma'):
    """
...
def my_reg_total_repair_reg(g, metric='sqeuclidean', reg=0.01, eta=1, log=False):
    """
    ...
    gamma = myreg_computeTransportSymmetric_fw(m, x_0, x_1, idx_p0, idx_p1, method='laplace', metric=metric,
                               reg=reg, nbitermax=5000,
...
def get_keys_from_value(d, val):
    return np.array([k for k, v in d.items() if v == val])

def multi_total_repair(g, num_iter=1e4, metric='sqeuclidean', reg=0.01, eta=1, log=False):
    """
    Repairing of the graph with OT and the sinkhorn version
    :param g: a graph to repair. The protected attribute is the node attribute
    :param metric: the distance metric for the cost matrix
...
#     s = np.fromiter(s.values(), dtype=int)
    ...
    # Separate rows adjacency matrix based on the protected attribute
    x_s = [np.array(x[idx_p]) for idx_p in idx_ps]

    # Get the barycenter between adj0 and adj1
    n_s = [myx_s.shape[0] for myx_s in x_s]
    d_s = [myx_s.shape[1] for myx_s in x_s]

    # Compute barycenters using POT library
    # Uniform distributions on samples
    weights = [np.ones((myn,))/myn for myn in n_s]
    X_init = np.random.normal(0., 1., (x.shape[0], x.shape[1]))
    # X_init = np.random.normal(0., 1., (x.shape[0], x.shape[1]))
    b = np.ones((X_init.shape[0],))/X_init.shape[0]
    start = time.time()
    X = ot.lp.free_support_barycenter(x_s, weights, X_init, b, numItermax=num_iter,stopThr=1e-5)
    print(f'free sbc lasted {time.time()-start} seconds')
    new_xs = []
    gammas = []
    ms = []
    for i in range(len(x_s)):
        m = ot.dist(x_s[i], X)
        ms.append(m)
        gamma = ot.emd(weights[i], b, m)
        gammas.append(gamma)
        new_xs.append(n_s[i]*np.dot(gamma, X))
        
    
    new_x = np.zeros(x.shape)
    i = 0
    for myidx in idx_ps: 
        new_x[myidx, :] = new_xs[i]
        i += 1
    
    return new_x, s, gammas, ms

def multi_sym_total_repair(g, metric='sqeuclidean', reg=1e-9, eta=1, log=False, num_iter=1e6,):
    """
    Repairing of the graph with OT and the sinkhorn version
    :param g: a graph to repair. The protected attribute is the node attribute
    :param metric: the distance metric for the cost matrix
...
#     s = np.fromiter(s.values(), dtype=int)
    ...
    # kwargs = {'sim': 'gauss', 'alpha': 0.5}
    start = time.time()
    X = sym_free_support_barycenter(x_s, weights, X_init, numItermax=num_iter, reg=reg)
    print(f'bc time lasted {time.time() - start} seconds')
    new_xs = []
    gammas = []
    ms = []
    ...
    for i in range(len(x_s)):
        ...
        gammas.append(gamma)
        new_xs.append(n_s[i]*np.dot(gamma, X))
        print(f'gamma {i} lasted {time.time()-start} seconds')
...
def sym_free_support_barycenter(measures_locations, measures_weights, X_init, reg_type='disp', reg_laplace=1e-1, reg_source=1,reg=1e-9,
                                    b=None, weights=None, numItermax=100, stopThr=1e-5, verbose=False, log=None,  metric = 'sqeuclidean'):
    """
    Solves the free support (locations of the barycenters are optimized, not the weights) Wasserstein barycenter problem (i.e. the weighted Frechet mean for the 2-Wasserstein distance)
    ...
    while (displacement_square_norm > stopThr and iter_count < numItermax):
        ...
        for (measure_locations_i, measure_weights_i, weight_i) in zip(measures_locations, measures_weights,
            ...
            T_i = multi_computeTransportSymmetric_fw(M_i, X, measure_locations_i, weight_i,method='laplace', metric=metric,
                               reg=reg,
                                solver=None, wparam=1)
...
def multi_node_sym_total_repair(g, metric='sqeuclidean', reg=1e-9, eta=1, log=False, num_iter=1e6,reg2=1, reg1=1):
    """
    Repairing of the graph with OT and the sinkhorn version
    :param g: a graph to repair. The protected attribute is the node attribute
    :param metric: the distance metric for the cost matrix
    ...
    x = nx.adjacency_matrix(g)
    x2 = nx.get_node_attributes(g, 'feat')
    s = nx.get_node_attributes(g, 's')
    idx_ps = []
    myx2 = []
    for val in set(s.values()):
...
#     s = np.fromiter(s.values(), dtype=int)
    ...
    x2s = [np.array(x2[idx_p]) for idx_p in idx_ps]
    ...
    weights = [np.ones((myn,))/myn for myn in n_s]
    num_classes = len(x_s)
    X_init = np.random.normal(0., 1., (x.shape[0], x.shape[1]+x2.shape[1]))
    ...
    # kwargs = {'sim': 'gauss', 'alpha': 0.5}
    start = time.time()
    X = sym_node_free_support_barycenter(x_s, x2s, weights, X_init, numItermax=num_iter, reg=reg, reg1=reg1, reg2=reg2, verbose=True)
    print(f'bc time lasted {time.time() - start} seconds')
    new_xs = []
    gammas = []
    ms = []
    d1, d2 = x_s[0].shape[1], x2s[0].shape[1]
    

    for i in range(len(x_s)):
        start = time.time() 
        myx = np.concatenate([reg1*x_s[i], reg2*x2s[i]], axis=-1)
        myres = np.empty((X.shape[0],myx.shape[0]))
        X[:d1] = reg1*X[:d1]
        X[d1:] = reg2*X[d1:]
        m = dist_arr_2(X, myx, myres)
        # m = reg1*ot.utils.dist(X[:,:d1],x_s[i] ) + reg2*ot.utils.dist(X[:,d1:], x2s[i])
        ms.append(m)
        gamma = ot.emd(b, weights[i], m).T
        gammas.append(gamma)
        new_xs.append(n_s[i]*np.dot(gamma, X))
        print(f'gamma {i} lasted {time.time()-start} seconds')
        
    
    new_x = np.zeros(X_init.shape)
    i = 0
    for myidx in idx_ps: 
        new_x[myidx, :] = new_xs[i]
        i += 1
    my_nodes = {}
    for i in range(new_x[:,d1:].shape[0]):
        my_nodes[i] = new_x[i, d1:]
    return new_x[:,:d1], new_x[:,d1:], s, gammas, ms

def sym_node_free_support_barycenter(measures_locations,measures_locations2, measures_weights, X_init, reg_type='disp', reg_laplace=1e-1, reg_source=1,reg=1e-9, reg1=1, reg2=1,
                                    b=None, weights=None, numItermax=100, stopThr=1e-7, verbose=False, log=None,  metric = 'sqeuclidean'):
    """
    Solves the free support (locations of the barycenters are optimized, not the weights) Wasserstein barycenter problem (i.e. the weighted Frechet mean for the 2-Wasserstein distance)
    ...
    d1, d2 = measures_locations[0].shape[1], measures_locations2[0].shape[1]
    if b is None:
        b = np.ones((k,))/k
    if weights is None:
        weights = np.ones((N,)) / N
    ...
    while (displacement_square_norm > stopThr and iter_count < numItermax):
        ...
        import time
        for (measure_locations_i, measure_locations2_i, measure_weights_i,  weight_i) in zip(measures_locations, measures_locations2, measures_weights, weights.tolist()):
            start = time.time()

            T_i = multi_node_computeTransportSymmetric_fw( X, measure_locations_i, measure_locations2_i, weight_i,method='laplace', 
                               reg=reg, reg1=reg1, reg2=reg2,
                                solver=None, wparam=1, nbitermax=100)
            print(f'fw calac lasted {time.time() - start} seconds')
            start = time.time()
            T_sum = T_sum + weight_i * np.reshape(1. / b, (-1, 1)) * np.matmul(T_i, np.concatenate([np.array(measure_locations_i), np.array(measure_locations2_i)], axis=-1))
            Ti.append(T_i)
            print(f'update calc lasted {time.time()-start}')
        displacement_square_norm = np.sum(np.square(T_sum-X))
        if log:
            displacement_square_norms.append(displacement_square_norm)
...
def multi_node_total_repair(g, metric='sqeuclidean', reg=1e-9, eta=1, log=False, num_iter=1e6,reg2=1, reg1=1):
    """
    Repairing of the graph with OT and the sinkhorn version
    :param g: a graph to repair. The protected attribute is the node attribute
    :param metric: the distance metric for the cost matrix
...
#     s = np.fromiter(s.values(), dtype=int)
    ...
    xs = []
    for i in range(len(x_s)):
        xs.append(np.concatenate([reg1*x_s[i], reg2*x2s[i]], axis=-1))
    # import pdb; pdb.set_trace()
    
    # Transport
    # kwargs = {'sim': 'gauss', 'alpha': 0.5}
    start = time.time()
    X = ot.lp.free_support_barycenter(xs, weights, X_init, b, numItermax=num_iter, verbose=True)
    # X = sym_node_free_support_barycenter(x_s, x2s, weights, X_init, numItermax=num_iter, reg=reg, reg1=reg1, reg2=reg2, verbose=True)
    print(f'bc time lasted {time.time() - start} seconds')
    new_xs = []
    gammas = []
